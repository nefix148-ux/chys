...
let units=[],enemies=[],projectiles=[],shields=[];
let waveTimer=30,waveNum=0,gameTime=0;
let selectedFilter='copper';
let core={x:0,y:0};
let buildMode='place';
...
function generateMap(){
  map=[];oreMap=[];buildMap=[];items=[];liquidItems=[];units=[];enemies=[];projectiles=[];shields=[];showCoreInv=false;ghosts=[];deleteSel.clear();waveTimer=30;waveNum=0;gameTime=0;
...
window.addEventListener('keydown',e=>{
  ...
  if(e.code==='KeyF'){const ids=RESOURCES.filter(r=>!r.liquid).map(r=>r.id);const i=ids.indexOf(selectedFilter);selectedFilter=ids[(i+1)%ids.length]}
...
function placeBuilding(tx,ty,type,dir){
  ...
  const b={type,x:tx,y:ty,dir:finalDir,items:{},liquids:{},progress:0,power:0,timer:0,health:def.health||100,maxHealth:def.health||100,reload:0,filter:selectedFilter};
