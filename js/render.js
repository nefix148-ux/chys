...
function draw(){
  ...
  for(const it of items){
    ...
    ctx.fillStyle='rgba(0,0,0,.3)';ctx.beginPath();ctx.arc(ix+1,iy+2,6,0,Math.PI*2);ctx.fill();
    ctx.fillStyle=res.color;ctx.beginPath();ctx.arc(ix,iy,6,0,Math.PI*2);ctx.fill();
    ctx.fillStyle='rgba(255,255,255,.4)';ctx.beginPath();ctx.arc(ix-1.5,iy-1.5,2,0,Math.PI*2);ctx.fill();
  }
